...
author: "Leanh Nguyen <Ln14> CEVE 421" # YOU SHOULD EDIT THIS
date: 2024-01-19
week: 2
categories: [Lab]
...
```{julia}
t_start = Dates.DateTime(2020, 3, 1, 0) # <1>
...
```{julia}
#| output: false
df[!, :day] = Dates.dayofyear.(df.datetime) # <1>
dropmissing!(df, :lsl) # <2>
df_byday = groupby(df, :day) # <3>
df_climatology = combine(df_byday, :lsl => mean => :lsl_avg); # <4>
```
```{julia}
plot(
    df_climatology.day, # <1>
    df_climatology.lsl_avg;
    tickfontsize=5,
    xticks=1:365, # <2>
    xlabel="Day",
